...
import (
	...
	"github.com/0disoft/actutum/internal/releaseevidence"
)
...
func run(args []string) int {
	flags := flag.NewFlagSet("actutum-release-evidence", flag.ContinueOnError)
	...
	if flags.NArg() != 0 || *directory == "" || *archive == "" || *commit == "" || *invocation == "" || *created == "" {
		fmt.Fprintln(os.Stderr, "actutum-release-evidence: release paths, source commit, invocation ID, and created time are required")
		return 2
	}
	createdAt, err := time.Parse(time.RFC3339, *created)
	if err != nil {
		fmt.Fprintln(os.Stderr, "actutum-release-evidence: invalid --created-at")
		return 2
	}
	...
	if err != nil {
		fmt.Fprintln(os.Stderr, "actutum-release-evidence:", err)
		return 6
	}
