...
- Status: Actutum rename implemented; public executable blocked pending repository identity
- Owner: Project maintainer

## Current State

Actutum has no published release, package registry entry, implemented signing
...
The first locally verified Actutum candidate is `0.6.0-alpha.1`. Its
`actutum-windows-x64.zip` SHA-256 is
`6df8d7ad2a81c9432dc53b2c16cd0c94a227ab718d8d3e8648c269c42fb315b7`.
This digest is local rename evidence only and must be replaced by same-commit
hosted evidence after the repository rename before publication.

ADR 0014 selects Actutum as the public product and executable identity. The
pipeline remains testable but publication is dormant until the GitHub
repository is actually `0disoft/actutum` and rebuilt evidence proves the
renamed artifact contract.

The repository also owns `actutum.signing-record/v1` and a non-publishable
...
Planned channels are alpha, beta, and stable. `0.6.0-alpha.1` is the internal
unsigned developer-preview candidate. Its eventual immutable tag remains
`v0.6.0-alpha.1`; its public artifacts and executables use the Actutum identity
selected under ADR 0014. Broader beta, stable, and support policy remain
UNDECIDED before M5.
...
- The public product and executable identity passes ADR 0014, the repository
  owns `0disoft/actutum`, and old-name artifact references are migrated or
...
Runtime `92.0.902.49` is the minimum because Actutum requires
...
The provider signs the reproducibly built `actutum.exe` and `actutum-host.exe`.
The repository-owned `actutum-signing-record prepare` command packages exactly
...
`actutum-host.json` and `release-manifest.json` describe signed bytes. The generic
...
`0disoft/actutum`. Do not create the candidate tag in the old repository or use
a redirect as a substitute for the final module and release identity.
...
- Public repository, module, or executable identity remains inconsistent.
