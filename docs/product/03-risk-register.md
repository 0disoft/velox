...
| R-014 | Static-only scope has too little user value | Medium | Critical | Open | ADR 0017 continues only narrow alpha work and closes M5 with zero independent-user attempts. Browser-owned dogfood and maintainer-controlled clean-room success are not demand; require external evidence or a later explicit zero-adoption risk acceptance before beta |
