...
| R-003 | Pure-Go COM lifecycle code is unsafe or costly | Medium | Critical | Monitoring | `docs/engineering/09-webview2-com-lifetime-review.md` records `runtime.Pinner` callback graphs, native reference-counted Go roots, failed/late initialization guards, and nine registration-failure ABI fixtures. Implementation `5926fba` passed fork tests and the local fresh/immediate/security Windows smoke; alpha.40 is the synchronized release candidate. Hosted candidate evidence, broader runtime coverage, and behavior when native references never balance remain distinct gates. These checks are not general memory-safety proof; reopen ADR 0005 if those controls cannot be represented safely |
