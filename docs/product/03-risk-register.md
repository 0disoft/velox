...
| R-003 | Pure-Go COM lifecycle code is unsafe or costly | Medium | Critical | Monitoring | `docs/engineering/09-webview2-com-lifetime-review.md` records `runtime.Pinner` callback graphs, native reference-counted Go roots, failed/late initialization guards, and nine registration-failure ABI fixtures. Implementation `5926fba` passed fork tests and local fresh/immediate/security smoke; published alpha.40 passed hosted reproducible-build, consumer, and public-startup checks. Public alpha.62 hosted 50-pair/100-launch run 35081507786 passed; broader Windows/WebView2 coverage and behavior when native references never balance remain distinct gates. Local alpha.49 readiness/browser-exit smoke failed once and passed unchanged on rerun; the intermittent failure remains unexplained. These checks are not general memory-safety proof; reopen ADR 0005 if those controls cannot be represented safely |
...
| R-008 | Full benchmark CI consumes excessive Actions resources | Medium | Medium | Mitigated | The public benchmark repository and the product consumer workflow have no recurring schedule trigger. Full evidence requires a benchmark or release-candidate tag or explicit manual dispatch; pull requests run bounded contract evidence, and recommended-cache runs use bounded keys with cleanup. Reopen if recurring execution or retained cache scope returns |
...
| R-013 | A downloaded release bundle is tampered with | Low | Critical | Monitoring | Public verifier run 35079337819 downloaded current preview `v0.5.10-alpha.62` without checkout and matched independently recorded ZIP SHA-256 `10137ca603c5ba7f765d58f9e93fc78683f328aebad77659fd63e01367265871` from publication producer run 35079091056 before exercising it. ADR 0011 does not pretend co-located checksums authenticate a compromised release channel; retain the unsigned-channel limitation and revisit authenticated attestations for a broader channel |
| R-014 | Static-only scope has too little user value | Medium | Critical | Open | ADR 0019 requires product workflow readiness but does not mitigate demand risk. Browser-owned dogfood, clean-room LLM trials, and maintainer-controlled release evidence are not human adoption; keep the zero-human signal visible in every channel decision |
...
| R-019 | LLM evaluation overfits the maintainer prompt or one model family | High | High | Monitoring | ADR 0019 makes AI evaluation optional, not channel authority. Retain ADR 0018 trial integrity rules and a false human-adoption claim for any historical or future optional series |
| R-020 | Session-log heuristics miss unrecorded filesystem or subprocess access | Medium | Critical | Mitigated | Hermes v1 remains non-qualifying. The Windows supervisor uses AppContainer explicit ACL grants plus a no-breakaway Job Object, sanitizes inherited environment data, binds the exact prompt and isolated state DB, emits v2 evidence only after successful cleanup, and has a local adversarial integration test. ADR 0019 removes model trials from channel admission; this supervisor test is not product or release evidence |
