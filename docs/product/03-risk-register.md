...
| R-003 | Pure-Go COM lifecycle code is unsafe or costly | Medium | Critical | Monitoring | `docs/engineering/09-webview2-com-lifetime-review.md` records pinned callback graphs, native reference-counted Go roots, failed/late initialization guards, and nine registration-failure ABI fixtures. Historical alpha.39 live lifecycle evidence does not validate the changed candidate. Candidate live/hosted validation, broader runtime coverage, and behavior when native references never balance remain distinct gates. These checks are not general memory-safety proof; reopen ADR 0005 if those controls cannot be represented safely |
