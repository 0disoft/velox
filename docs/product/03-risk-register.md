...
| R-014 | Static-only scope has too little user value | Medium | Critical | Open | ADR 0008 limits the answer to offline portable deterministic artifacts. M5 starts with zero independent-user attempts; do not treat maintainer-controlled clean-room success as demand, and require external evidence or an explicit zero-adoption risk acceptance before beta |
