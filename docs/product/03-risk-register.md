...
| R-017 | Signing obscures the reproducible unsigned lineage | Medium | Critical | Open | Preserve byte-identical unsigned evidence, bind provider request and signed output digests in a versioned signing record, assemble the final manifest from signed bytes, and attest only the final distribution |
