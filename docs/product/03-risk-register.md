...
| R-003 | Pure-Go COM lifecycle code is unsafe or costly | Medium | Critical | Monitoring | The independent bounded source review in `docs/engineering/09-webview2-com-lifetime-review.md` fixed leaked settings references and incomplete failed-initialization teardown, with focused success and failure tests. Retained Go callback addresses still lack an explicit `runtime.Pinner` contract, and repeated live Windows/WebView2 stress remains required before beta; reopen ADR 0005 if those controls cannot be represented safely |
