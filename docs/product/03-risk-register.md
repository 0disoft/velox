...
| R-003 | Pure-Go COM lifecycle code is unsafe or costly | Medium | Critical | Monitoring | The bounded source review in `docs/engineering/09-webview2-com-lifetime-review.md` fixed leaked settings references and incomplete failed-initialization teardown. On 2026-09-08 the fork tests, root tests and vet, and ten local fresh/immediate Windows lifecycle pairs passed on WebView2 `152.0.4191.66`. Retained Go callback addresses still lack an explicit pinning contract; registration-failure injection and broader runtime coverage remain unverified. These local checks are not general memory-safety proof; reopen ADR 0005 if those controls cannot be represented safely |
