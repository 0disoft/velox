...
`29736140250`. Its source repository is now archived as a one-shot receipt; no
...
| SEC-004 | Medium | Source-reviewed; monitoring residual risk | The independent bounded review in `docs/engineering/09-webview2-com-lifetime-review.md` traced native owners and release paths and fixed three defects: leaked settings references, settings-stage failures that queued close without pumping the message loop, and partial embed failures without unconditional browser teardown. Focused tests now cover settings release, failed initialization cleanup, repeated shutdown, and callback-after-close. Retained Go callback addresses still lack an explicit `runtime.Pinner` contract, and live repeated Windows/WebView2 stress remains required before beta. |
...
SEC-004 completed its bounded source review while retaining explicit native
callback pinning and live stress risks for beta.
...
- The Go collector, syscall pointer rules, or retained-callback allocation
  strategy changes.
