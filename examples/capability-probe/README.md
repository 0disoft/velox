...
a file, or change the clipboard during startup. User actions replace detection
states with `passed`, `canceled`, `blocked`, or `failed` outcomes. The saved JSON
contains environment metadata and result summaries, never selected file bytes.
