...
func TestInspectValidatesDirectoryAndZIP(t *testing.T) {
	...
	for _, test := range []struct {
		...
		if result.Kind != test.kind || result.ReleaseVersion != "0.5.10-alpha.57" || result.App.ID != "com.example.inspect" || result.PortableFiles != 4 {
...
func buildFixture(t *testing.T) builder.Result {
	...
	metadata := fmt.Sprintf(`{"schemaVersion":"velox.host/v1","releaseVersion":"0.5.10-alpha.57","target":"windows-x64","contracts":{"host":1,"runtime":1,"ipc":1},"host":{"file":"velox-host.exe","bytes":%d,"sha256":"%x"}}`, len(host), digest)
