...
func TestSnapshotCannotMutatePlan(t *testing.T) {
	...
	writePlanFile(t, filepath.Join(root, "actutum-host.exe"), "host")
	writePlanHostMetadata(t, root, "host")
	writePlanFile(t, filepath.Join(root, "actutum.json"), `{
  "schemaVersion": 1,
...
func TestCreateRejectsOutputContainingAssets(t *testing.T) {
	...
	writePlanFile(t, filepath.Join(root, "actutum-host.exe"), "host")
	writePlanHostMetadata(t, root, "host")
	writePlanFile(t, filepath.Join(root, "actutum.json"), `{"schemaVersion":1,"app":{"id":"com.example.hello","name":"Hello","version":"1"}}`)
...
func TestCreateUsesFullApplicationIDAsCollisionFreeOutputKey(t *testing.T) {
	root := t.TempDir()
	host := []byte("host")
	writePlanFile(t, filepath.Join(root, "release", "actutum-host.exe"), string(host))
	writePlanHostMetadata(t, filepath.Join(root, "release"), string(host))
	writePlanFile(t, filepath.Join(root, "web", "index.html"), "ok")
	writePlanFile(t, filepath.Join(root, "actutum.json"), `{"schemaVersion":1,"app":{"id":"com.example.hello","name":"Hello","version":"1"}}`)
	plan, err := Create(Options{ManifestPath: filepath.Join(root, "actutum.json"), HostPath: filepath.Join(root, "release", "actutum-host.exe")})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateCanonicalizesRedirectedHostAncestor(t *testing.T) {
	...
	writePlanFile(t, filepath.Join(root, "web", "index.html"), "ok")
	writePlanFile(t, filepath.Join(root, "actutum.json"), `{"schemaVersion":1,"app":{"id":"com.example.hello","name":"Hello","version":"1"}}`)
	writePlanFile(t, filepath.Join(target, "actutum-host.exe"), "host")
	...
	plan, err := Create(Options{
		ManifestPath: filepath.Join(root, "actutum.json"),
		HostPath:     filepath.Join(redirect, "actutum-host.exe"),
		OutputRoot:   filepath.Join(root, "dist"),
	})
	if err != nil {
		t.Fatal(err)
	}
	want, err := canonicalPath(filepath.Join(target, "actutum-host.exe"))
	if err != nil {
		t.Fatal(err)
	}
...
func writePlanHostMetadata(t *testing.T, root, host string) {
	...
	body := fmt.Sprintf(`{"schemaVersion":"actutum.host/v1","releaseVersion":"0.6.0-alpha.1","target":"windows-x64","contracts":{"host":1,"runtime":1,"ipc":1},"host":{"file":"actutum-host.exe","bytes":%d,"sha256":"%x"}}`, len(host), digest)
	writePlanFile(t, filepath.Join(root, "actutum-host.json"), body)
}
