...
func TestBuildJSONContractAndOutputs(t *testing.T) {
	...
	if result.ReleaseVersion != "0.5.10-alpha.60" || result.AppID != "com.example.hello" || result.AppVersion != "1.0.0" || result.Target != "windows-x64" {
...
func TestValidateVerboseIsBoundedAndQuietWins(t *testing.T) {
	...
	want := "velox: detail: release=0.5.10-alpha.60 target=windows-x64 assets=1 permissions=0\n"
...
func cliFixture(t *testing.T) (string, string, string) {
	...
	writeCLIFile(t, filepath.Join(filepath.Dir(host), "velox-host.json"), fmt.Sprintf(`{"schemaVersion":"velox.host/v1","releaseVersion":"0.5.10-alpha.60","target":"windows-x64","contracts":{"host":1,"runtime":1,"ipc":1},"host":{"file":"velox-host.exe","bytes":4,"sha256":"%x"}}`, digest))
