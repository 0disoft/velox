...
func TestBuildProducesDeterministicChecksumsSBOMAndProvenance(t *testing.T) {
	...
	options := Options{ReleaseDirectory: release.Directory, ReleaseArchive: release.Archive, SourceRepository: "https://github.com/0disoft/actutum", SourceCommit: strings.Repeat("a", 40), InvocationID: "test-1", CreatedAt: created}
...
func TestBuildRejectsArtifactTampering(t *testing.T) {
	release := buildReleaseFixture(t)
	if err := os.WriteFile(filepath.Join(release.Directory, "actutum.exe"), []byte("tampered"), 0o644); err != nil {
		t.Fatal(err)
	}
	_, err := Build(Options{ReleaseDirectory: release.Directory, ReleaseArchive: release.Archive, OutputRoot: filepath.Join(t.TempDir(), "evidence"), SourceRepository: "https://github.com/0disoft/actutum", SourceCommit: strings.Repeat("a", 40), InvocationID: "test-2", CreatedAt: time.Now().UTC()})
...
func buildReleaseFixture(t *testing.T) releasebundle.Result {
	...
	writeEvidenceFixture(t, filepath.Join(input, "actutum.exe"), []byte("cli"))
	writeEvidenceFixture(t, filepath.Join(input, "actutum-host.exe"), []byte("host"))
	for _, name := range []string{"build-result-v1.schema.json", "consumer-clean-v1.schema.json", "host-metadata-v1.schema.json", "ipc-v1.schema.json", "public-preview-verification-v1.schema.json", "release-manifest-v1.schema.json", "runtime-config-v1.schema.json", "actutum-v1.schema.json"} {
	...
	result, err := releasebundle.Build(releasebundle.Options{CLIPath: filepath.Join(input, "actutum.exe"), HostPath: filepath.Join(input, "actutum-host.exe"), SourceRoot: source, OutputRoot: filepath.Join(root, "release")})
	if err != nil {
		t.Fatal(err)
	}
	return result
